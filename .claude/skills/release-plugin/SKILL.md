...
### 1. Pre-flight
```bash
git config user.email   # must be the personal noreply, NOT @<work-domain>
...
```bash
git grep -nE '<internal-ip>|<internal-ip>|otel\.<org>-tech\.com|v2\.monitoring\.<org>\.work|<ticket-id>|@<org>\.(com|work)|glsa_|glc_' -- . ':!package-lock.json' || echo "clean"
```
Any hit → stop and redact before continuing.
